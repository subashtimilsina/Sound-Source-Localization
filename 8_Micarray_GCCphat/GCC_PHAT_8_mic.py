...
def find_direction_gccphat(data_frame1,data_frame2,fs):
    ...
    #tau2 = gccphat(mic3,mic2,fs=fs,interp=16)

    #azimuth1 = theta_intersection(tau1,tau2)

    #############Azimuth ----------------2

    #tau1 = gccphat(mic5,mic8,fs=fs,interp=16)
    #tau2 = gccphat(mic7,mic6,fs=fs,interp=16)

    #azimuth2 = theta_intersection(tau1,tau2)

    # if azimuth2 >= 330:
    #     azimuth2 = azimuth2 - 330
    # else:
    #     azimuth2 = azimuth2 + 30

    ########################################################################
    #azimuth1 = np.arctan(np.tan(np.deg2rad(elevation1))*np.tan(np.deg2rad(elevation2)))
    #theta1 = int(np.rad2deg(azimuth1))

    #theta = 1 - np.power(np.cos(np.deg2rad(elevation1)),2) - np.power(np.cos(np.deg2rad(elevation2)),2)  
    #theta = np.rad2deg(np.arccos(np.sqrt(theta)))
    ########################################################################


    final_theta = (elevation1,elevation2)#,azimuth1,azimuth2)

    return final_theta
    
def find_direction_12_gccphat(data_frame1,data_frame2,data_frame3,fs):
    

    ############ Elevation--------------1 --- YZ plane 
    tau1 = gccphat(data_frame1[0::4],data_frame1[1::4],fs=fs,interp=16)
    tau2 = gccphat(data_frame1[2::4],data_frame1[3::4],fs=fs,interp=16)
    
    elevation1 = theta_intersection(tau1,tau2) 

    ################ Elevation-----------2 --- XY plane 

    tau1 = gccphat(data_frame2[0::4],data_frame2[1::4],fs=fs,interp=16)
    tau2 = gccphat(data_frame2[2::4],data_frame2[3::4],fs=fs,interp=16)
    
    elevation2 = theta_intersection(tau1,tau2) 

    ################ Azimuth-----------2 --- XZ plane 

    tau1 = gccphat(data_frame3[0::4],data_frame3[3::4],fs=fs,interp=16)
    tau2 = gccphat(data_frame3[1::4],data_frame3[2::4],fs=fs,interp=16)
    
    azimuth = theta_intersection(tau1,tau2) 


    #################### Final Output #############################
    final_theta = (elevation1,elevation2,azimuth)
...
def Cube_8_test():
    ...
    with MicArray(device_index= 7,channels=4) as array1,MicArray(device_index=8 ,channels=4) as array2,MicArray(device_index=9 ,channels=4) as array3:
        for chunk1,chunk2,chunk3 in zip(array1.read_mic_data(),array2.read_mic_data(),array3.read_mic_data()):
            direction = find_direction_gccphat(chunk1,chunk2,chunk3,fs=array1.rate)
            print(direction)
            #f.writelines(str(direction)[1:-1]+"\n")
...
def Cube_4_test():
    ...
    with MicArray(device_index=6 ,channels=4) as array1:
